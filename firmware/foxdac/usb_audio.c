...
static void __not_in_flash_func(process_audio_packet)(const uint8_t *data, uint16_t data_len) {
    ...
    if (!is_bypassed) {
        if (!channel_bypassed[CH_MASTER_LEFT]) {
            dsp_process_channel_block_svf(filters[CH_MASTER_LEFT], buf_l, sample_count, CH_MASTER_LEFT);
        }
        if (!channel_bypassed[CH_MASTER_RIGHT]) {
            dsp_process_channel_block_svf(filters[CH_MASTER_RIGHT], buf_r, sample_count, CH_MASTER_RIGHT);
        }
    }
    ...
    if (core1_mode == CORE1_MODE_EQ_WORKER) {
        ...
        for (int out = 0; out < CORE1_EQ_FIRST_OUTPUT; out++) {
            ...
            if (!matrix_mixer.outputs[out].mute) {
                ...
                if (!channel_bypassed[eq_ch]) {
                    dsp_process_channel_block_svf(filters[eq_ch], buf_out[out], sample_count, eq_ch);
                }
            }
        ...
        start_systick();
        unsigned current_rounding = fegetround();
        for (uint32_t i = 0; i < sample_count; i++) {
        ...
        if (audio_buf[0]) {
            ...
            if (!matrix_mixer.outputs[left_ch].enabled && !matrix_mixer.outputs[right_ch].enabled) {
                ...
                for (uint32_t i = 0; i < sample_count; i++) {
                    ...
                    buf_out[left_ch][i] = dl;
                    buf_out[right_ch][i] = dr;
                }
                ...
                for (uint32_t i = 0; i < sample_count; i++) {
                    out_ptr[i*2]     = (int16_t)rintf(buf_out[left_ch][i]);
                    out_ptr[i*2+1]   = (int16_t)rintf(buf_out[right_ch][i]);
                }
            }
        }
